...
unsigned int* computeOnGPU(unsigned int * input, int width, int height, int thresh){
    ...
    /* printf("%d\n%d",width,height); */
    /* printf("gridWidth: %d,gridHeight%d\n",(height-1 + BLOCKSIZE)/BLOCKSIZE,(height-1 + BLOCKSIZE)/BLOCKSIZE); */
    /* dim3 dimGrid((width-1+BLOCKSIZE)/BLOCKSIZE,(height-1 + BLOCKSIZE)/BLOCKSIZE); */
    /* dim3 dimBlock(BLOCKSIZE,BLOCKSIZE); */
    /* ppmKernel<<<dimGrid,dimBlock>>>(d_input, d_output,width,height,thresh); */
    /* ppmKernel<<<1,1>>>(d_input, d_output,width,height,thresh); */
    /* cudaMemcpy(h_output, d_output,size,cudaMemcpyDeviceToHost); */
    /* cudaMalloc(&d_input,1); */
    /* cudaMalloc(&d_output,1); */
    test<<<1,10>>>();
    /* cudaMemcpy(h_output,d_output,1,cudaMemcpyDeviceToHost); */
    cudaFree(d_input);
    cudaFree(d_output);
    printf("%d" ,h_output[0]);
    /* for (int i = 0; i < width*height; ++i) { */
